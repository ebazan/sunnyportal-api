...
var CURRENT_PRODUCTION_URL_HOMEMANAGER = '/homemanager?_=1';
var CURRENT_PRODUCTION_URL_PORTAL = '/Dashboard?_=1';
var CURRENT_PRODUCTION_URL = CURRENT_PRODUCTION_URL_PORTAL;
var DOWNLOAD_RESULTS_URL = '/Templates/DownloadDiagram.aspx?down=diag';
var SESSION_MAX_AGE = 600000;
...
var SunnyPortal = function(opts) {
  ...
  var plantType = opts.plantType || 'portal';
	...
	var _login = function(callback) {
      ...
      var page = (plantType === 'homemanager') ? 'HoManLive' : 'Dashboard';
			if(httpResponse.headers.location && httpResponse.headers.location=='/FixedPages/' + page + '.aspx') {
				callback(err, jar);
			} else {
  ...
  var _loginHandler = function(callback) {
    if (!_loginSession || _loginSession.jar === null || _loginSession.date < new Date() - SESSION_MAX_AGE) {
      _loginSession = {
        date: new Date(),
        jar: null
      };
      ...
      _login(function(err, jar) {
        if (err || !jar) {
          ...
          _loginSession.jar = jar;
          callback(null, _loginSession.jar);
        }
      });
    }
    else {
      console.log('reusing login');
      callback(null, _loginSession.jar);
    }
  };

  _logout = function() {
    _loginSession = undefined;
  };

  var _loginSession = undefined;
	...
	*/
    ...
    _loginHandler(function(err, jar) {
			if(err) {
				callback(err);
			}
			...
			//The timestamp is just ignored. Using 1. (homemanager needs the timtestamp to work, readding it here)
			request(url + ((plantType === 'homemanager') ? CURRENT_PRODUCTION_URL_HOMEMANAGER : CURRENT_PRODUCTION_URL_PORTAL), requestOpts, function (err, httpResponse, body) {
				if (err) {
          _logout();
